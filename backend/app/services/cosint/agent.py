...
COSINT_AGENT_MODEL = "gpt-4o-mini"
...
def get_cosint_agent(streaming: bool = False):
    ...
    llm = ChatOpenAI(model=COSINT_AGENT_MODEL, temperature=0, streaming=streaming)
    tools = [
        MemberSearchTool(), 
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "Contextual Hint: {context}\n\n"
                   ...
                   "- For specific bills, always link to the internal COSINT bill page using this format: [BILL_TYPE BILL_NUMBER](/bill/CONGRESS/BILL_TYPE/BILL_NUMBER).\n"
                   "- Do NOT append extra links like 'Member Name's Profile' at the end of your response.\n"
                   "- ACTION TRIGGER (MEMBER): Whenever your response discusses, identifies, or provides information about a specific Congress member (Senator or Representative), you MUST ALWAYS append this tag at the very end of your response: [CREATE_PAGE_ACTION: Member Name | BIOGUIDE_ID]. This applies to any query where you look up, describe, or reference a specific member by name — including 'Who is...', biographical questions, voting records, policy positions, news, etc. Never omit this tag if a specific member was discussed.\n"
                   "- ACTION TRIGGER (BILL): If you have identified a specific bill the user is researching or asking about, ALWAYS append this tag at the very end: [RESEARCH_BILL: Congress | Type | Number | Title]\n"
                   "- Ensure links are descriptive (e.g., [S.Res.101](/bill/118/sres/101) instead of just [Link](URL)).\n"
                   "- If possible, prefer internal links over congress.gov for bills."
...
def get_intel_extraction_agent():
    ...
    """
    llm = ChatOpenAI(model=COSINT_AGENT_MODEL, temperature=0)
